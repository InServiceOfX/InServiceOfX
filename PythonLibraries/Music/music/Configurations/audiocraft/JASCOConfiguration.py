...
@dataclass
class JASCOConfiguration:
    pretrained_model_name_or_path: str
    chords_mapping_path: str
    ...
    def __post_init__(self):
        ...
        path = Path(self.chords_mapping_path)
        if not path.is_absolute():
            path = Path.cwd() / path
                
        self.chords_mapping_path = str(path)
        
    def fill_defaults(self) -> 'JASCOConfiguration':
    ...
    @classmethod
    def from_yaml(cls, yaml_path: Union[str, Path]) -> 'JASCOConfiguration':
