...
class LearnedTanh(nn.Module):
    def __init__(self, slope=1, n=1):
        ...
        self.slope = torch.Tensor(1).fill_(slope)
        self.n = torch.ones(1)

    def initialize_with_device(self, device, n=1):
        self.slope = nn.Parameter(self.slope.to(device))
        self.n = self.n.to(device) * n
