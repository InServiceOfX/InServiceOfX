...
def test_print_as_html_formatted_text(printer):
    with patch('clichat.Utilities.Printing.print_formatted_text') as mock_print:
        printer.print_as_html_formatted_text("Test content")
        mock_print.assert_called_once()
        assert f"<{printer._configuration.terminal_PromptIndicatorColor2}>" in \
            mock_print.call_args[0][0].value

def test_print_key_value_with_valid_format(printer):
    with patch('clichat.Utilities.Printing.print_formatted_text') as mock_print:
        printer.print_key_value("key: value")
        mock_print.assert_called_once()
        html_content = mock_print.call_args[0][0].value
