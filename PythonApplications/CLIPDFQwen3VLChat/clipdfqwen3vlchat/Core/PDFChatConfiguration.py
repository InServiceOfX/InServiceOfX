...
class PDFChatConfiguration(BaseModel):
    ...
    output_path: Path = Field(
        ...,
        description="Directory to write Qwen3-VL responses into.",
    )
    prompt: str = Field(
        ...,
        description=(
            "The question / instruction the model is asked about every page. "
            "Sent verbatim as the user-role text content alongside the page "
            "image."
        ),
    )
    pdf_dpi: int = Field(
    ...
    image_format: str = Field(
        "PNG",
        description=(
            "Format for saved page images (only if save_intermediate_images)."
        ),
    )
    save_intermediate_images: bool = Field(
        True,
        description="Persist rasterized page PNGs alongside responses.",
    )
    skip_existing: bool = Field(
        True,
        description="Skip pages whose .txt output already exists.",
    )
