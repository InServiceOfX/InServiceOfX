...
class ProcessConfigurations:
    ...
    def process_configurations(self) -> None:
        ...
        if not pdf_path.exists():
            raise FileNotFoundError(
                f"pdf_index_configuration.yml not found at {pdf_path}. "
                "Copy pdf_index_configuration.yml.example and edit."
            )
        self.configurations["pdf_index_configuration"] = (
            PDFIndexConfiguration.from_yaml(pdf_path)
        )
        print(f"Loaded pdf_index_configuration from {pdf_path}")
