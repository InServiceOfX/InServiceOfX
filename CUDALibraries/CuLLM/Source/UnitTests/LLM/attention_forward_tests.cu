...
TEST_F(AttentionForwardTests, BasicFunctionality)
{
  ...
  const dim3 grid_size(
    (batch_size * num_heads * seq_length * seq_length + block_size.x - 1) /
      block_size.x);
...
TEST_F(AttentionForwardTests, MathematicalProperties)
{
  ...
  for (int t1 {0}; t1 < seq_length; ++t1)
  {
    for (int t2 {0}; t2 <= t1; ++t2)
    {
      ...
      const float expected {compute_expected_dot_product(query, key, head_size)};
...
TEST_F(AttentionForwardTests, MultipleBatchAndHeads)
{
  ...
  for (const auto& block_size : block_sizes)
  {
    ...
    const float expected {hidden_size / (2.0f * std::sqrt(hidden_size / 2.0f))};
